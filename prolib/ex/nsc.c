...
main () {
  ...
  for (i=1; i<=NAT; i++) {
    ...
    co[3*i-2] = j*1*c;
    co[3*i-1] = j*1*c;
// co[3*i-3] = i*1.4; co[3*i-2] = 0.; co[3*i-1] = 0.;
// co[3*i-2] = a*0.3; a = -a; b=0; if (i%3 == 0) b=0.5; co[3*i-1] = b; ra[i-1] = 2.0;
    ra[i-1] = (1.+j*0.5)*c;
    }
...
#endif*/
const int matm=3000;
char ANAM_std[] = "CNOSPH";
double RADIUS_std[] = {1.91, 1.82, 1.70, 2., 2.1, 0.};
int na, nr, ridx[matm];
vector<string> resinfo;
double rads[matm], x[matm][3];
void rdpdb(string fn){
	FILE *fp = openfile(fn, "r");
	char str[201]; string resinfo0;
	na = nr = 0; resinfo.clear();
	while(fgets(str, 200, fp) != NULL){
		if(strstr(str, "ATOM") != str) continue;
		string r1 = string(str).substr(17, 10);
		if(r1 != resinfo0){
			resinfo0 = r1; nr ++;
			resinfo.push_back(r1);
		}
		char an = str[13];
		if(an == 'H') continue;
		char *strs = strchr(ANAM_std, an);
		if(strs == NULL) die("unknown atom: %c\n%s\n", an, str);
		rads[na] = RADIUS_std[strs-ANAM_std] + 1.4;
		double *xt = x[na];
...
//		printf("%d %f\n", i, rads[i]);
	...
	printf("# %s: nres %d; natm %d\n", fn.c_str(), nr, na);
}
void calsurface(){
	double w[na], rsa[nr];
	double *area, s0, volume, *dots;
	int ndots;
	for(int i=0; i<na; i++) w[i] = 1.;
//	surface(na, x[0], rads, w, area);
	if (NSC(x[0], rads, na, 42, FLAG_ATOM_AREA, &s0,
		&area, NULL, NULL, NULL)) ERROR("error in NSC");
	...
	for(int i=0; i<nr; i++){
		printf("%s %.1f\n", resinfo[i].c_str(), rsa[i]);
	}
	if(FLAG_ATOM_AREA) free(area);
}
